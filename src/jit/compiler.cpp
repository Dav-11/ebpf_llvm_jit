...
void context::loadLddwHelpers(const std::vector<std::string> &lddwHelpers) {
     ...
     */
    lddwHelperWithUint32 =FunctionType::get(
        Type::getInt64Ty(*llvmContext),
        { Type::getInt32Ty(*llvmContext) },
        false
    );

    lddwHelperWithUint64 = FunctionType::get(
        Type::getInt64Ty(*llvmContext),
        { Type::getInt64Ty(*llvmContext) },
        false
    );
    ...
    for (const auto &helperName : lddwHelpers) {
        ...
        if (helperName == LDDW_HELPER_MAP_VAL) {
            func = Function::Create(lddwHelperWithUint64,Function::ExternalLinkage,
                                    helperName, jitModule.get());

        } else {
            func = Function::Create(lddwHelperWithUint32,Function::ExternalLinkage,
                                    helperName, jitModule.get());
        }
...
void context::loadExtFuncs(const std::vector<std::string> &extFuncNames)
{
    // Define ext functions types  (does not add them to LLVM IR)
    helperFuncTy = FunctionType::get(
            Type::getInt64Ty(*llvmContext),
            {
                    Type::getInt64Ty(*llvmContext),
                    Type::getInt64Ty(*llvmContext),
        ...
        bool patch_map_val_at_compile_time)
{
    ...
    // load ext functions
    this->loadLddwHelpers(lddwHelpers);
    this->loadExtFuncs(extFuncNames);

    // Initializes a vector of booleans, with size equal to the number of instructions.
    // All values are initially set to false.
    blockBegin = std::vector<bool>(insts.size(), false);
    ...
    for (uint16_t i = 0; i < insts.size(); i++) {
        ...
        // set current insn to block_begin if previous is a jump
    ...
    Function *bpf_func = Function::Create(
            FunctionType::get(
                    Type::getInt64Ty(*llvmContext),
                    {
                        llvm::PointerType::getUnqual(llvm::Type::getInt8Ty(*llvmContext)),
                        Type::getInt64Ty(*llvmContext)
                    },
                    false
            ),
    ...
    Value *callStack, *callItemCnt;
    {
        ...
        setupBlock = BasicBlock::Create(*llvmContext, "setupBlock", bpf_func);
        ...
        for (uint16_t i = 0; i < insts.size(); i++) {
            if (blockBegin[i]) {
                ...
                if (i > 1 &&
                    ...
                    auto blockAddr = llvm::BlockAddress::get(bpf_func, currBlk);
    ...
    exitBlock = BasicBlock::Create(*llvmContext, "exitBlock", bpf_func);
    {
        IRBuilder<> builder(exitBlock);
        builder.CreateRet(builder.CreateLoad(builder.getInt64Ty(), regs[0]));
    }

    // Basic blocks that handle the returning of local func
    localRetBlock = BasicBlock::Create(*llvmContext, "localFuncReturnBlock", bpf_func);
    {
        // The most top one is the returning address, followed by r6,
        // r7, r8, r9
        IRBuilder<> builder(localRetBlock);
        ...
        builder.CreateStore(
                builder.CreateAdd(
                        builder.CreateLoad(builder.getInt64Ty(),regs[10]),
    ...
    for (uint16_t pc = 0; pc < insts.size(); pc++) {
        ...
        if (blockBegin[pc]) {
            if (auto itr = instBlocks.find(pc); itr != instBlocks.end()) {
                currBB = itr->second;
            } else {
    ...
    return ThreadSafeModule(std::move(jitModule), std::move(llvmContext));
}
